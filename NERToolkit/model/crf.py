...
BOS = NERToolkit.Constants.BOS
EOS = NERToolkit.Constants.EOS
...
class CRF(nn.Module):
    ...
    def __init__(self, vocabs, args):
        ...
        self.PAD_IDX = vocabs["tag"].lookup(PAD)
        self.BOS_IDX = vocabs["tag"].lookup(BOS)
        self.EOS_IDX = vocabs["tag"].lookup(EOS)

        self.trans = nn.Parameter(torch.randn(self.tag_size, self.tag_size), requires_grad=True)
        self.trans.data[self.BOS_IDX, :] = -10000
        self.trans.data[:, self.EOS_IDX] = -10000
    ...
    def _forward_alg(self, lstm_out, mask):
        ...
        alpha = lstm_out[:, 0, :] + trans[:, :, self.BOS_IDX]
        ...
        alpha = alpha + self.trans[self.EOS_IDX, :]  # [B]
    ...
    def _gold_score(self, lstm_out, tag_seq_tensor, word_seq_lens, mask):
        ...
        start_col = torch.full((batch_size, 1), self.BOS_IDX, dtype=torch.long).to(self.device)
        ...
        trans_to_end = self.trans[torch.LongTensor([self.EOS_IDX] * batch_size), last_tags]  # [B]
    ...
    def neg_log_likelihood(self, lstm_out, data):
        ...
        gold_score = self._gold_score(lstm_out, tag_seq_tensor,
                                      word_seq_lens, mask)
    ...
    def decode(self, lstm_out, mask):
        ...
        last_to_stop = self.trans[self.EOS_IDX, :].unsqueeze(0)  # [1 x C]
